...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Hot Zone Analysis on each rectangle by grouping them by rectangle
    val result = spark.sql("select rectangle, count(point) from joinResult group by rectangle order by rectangle")
